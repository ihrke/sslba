...
make.parlists <- function(p,sv=1,gf=1e-6,tf=1e-6) {
  ...
  p <- untrans(p)     
  list(
      D1S1=list(ter=rep(p["ter"],2),
        A=rep(p["A"],2),
        sv=rep(sv,2),
        b=p["A"]+rep(p["B"],2),
        v=c(p["V"],p["v"]),
        gf=gf,tf=tf),
      D1S2=list(ter=rep(p["ter"],2),
        A=rep(p["A"],2),
        sv=rep(sv,2),
        b=p["A"]+rep(p["B"],2),
        v=c(p["v"],p["V"]),
        gf=gf,tf=tf),
      D2S1=list(ter=rep(p["ter"],2),
        A=rep(p["A"],2),
        sv=rep(sv,2),
        b=p["A"]+rep(p["B"],2),
        v=c(p["V"],p["v"]),
        gf=gf,tf=tf),           
      D2S2=list(ter=rep(p["ter"],2),
        A=rep(p["A"],2),
        sv=rep(sv,2),
        b=p["A"]+rep(p["B"],2),
        v=c(p["v"],p["V"]),
        gf=gf,tf=tf),
      D1S1s=list(ter=p[c("ster","ter", "ter")],
        A=rep(p["A"],3),
        sv=rep(sv,3),
        b=p["A"]+rep(p["B"],3),
        v=p[c("Vs","V","v")],
        gf=gf,tf=tf),
      D1S2s=list(ter=p[c("ster","ter", "ter")],
        A=rep(p["A"],3),
        sv=rep(sv,3),
        b=p["A"]+rep(p["B"],3),
        v=p[c("Vs","v","V")],
        gf=gf,tf=tf),
      D2S1s=list(ter=p[c("ster","ter", "ter")],
        A=rep(p["A"],3),
        sv=rep(sv,3),
        b=p["A"]+rep(p["B"],3),
        v=p[c("Vs","V","v")],
        gf=gf,tf=tf),           
      D2S2s=list(ter=p[c("ster","ter","ter")],
        A=rep(p["A"],3),
        sv=rep(sv,3),
        b=p["A"]+rep(p["B"],3),
        v=p[c("Vs","v","V")],
